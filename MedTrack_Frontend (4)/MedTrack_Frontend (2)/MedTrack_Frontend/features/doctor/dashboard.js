document.addEventListener('DOMContentLoaded', function () {
  ...
  if (todayTableBody) {
    if (todaysAppts.length === 0) {
      ...
      for (var ti = 0; ti < sorted.length; ti++) {
        ...
        rows += '<tr><td class="text-muted">' + timeDisplay + '</td><td class="fw-medium text-dark">' + (a.patientName || 'Unknown') + '</td><td class="text-muted">' + (a.reasonForVisit && a.reasonForVisit.length > 15 ? a.reasonForVisit.substring(0, 15) + '...' : a.reasonForVisit || 'Consultation') + '</td><td class="text-end"><span class="status-badge ' + statusClass + '">' + statusLabel + '</span></td></tr>';
      }
  ...
  if (headerBadge) headerBadge.textContent = notifCount;
  if (sidebarBadge) sidebarBadge.textContent = notifCount;
  ...
  if (searchInput && searchDropdown) {
    ...
    for (var si = 0; si < allPatients.length; si++) {
      ...
      searchData.push({ id: p.id, type: 'patient', name: p.name, detail: p.department + ' | ' + p.contact, link: '../doctor/patient-records.html' });
    }
    ...
    for (var si2 = 0; si2 < recentApptsForSearch.length; si2++) {
      ...
      searchData.push({ id: sa.id, type: 'appointment', name: (sa.patientName || 'Patient') + ' (' + sa.id + ')', detail: sa.appointmentDate + ' ' + sa.startTime + ' - ' + (sa.reasonForVisit || 'Consultation'), link: 'appointments.html' });
    }
    ...
    var defaultStateHTML = '<li><h6 class="dropdown-header text-muted fw-bold">Quick Links</h6></li>' +
      '<li><a class="dropdown-item d-flex align-items-center gap-2 py-2" href="appointments.html"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg> View my appointments</a></li>' +
      '<li><a class="dropdown-item d-flex align-items-center gap-2 py-2" href="patient-records.html"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg> Search patient records</a></li>';
