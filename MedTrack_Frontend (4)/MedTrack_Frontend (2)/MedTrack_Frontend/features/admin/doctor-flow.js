...
function handleStep1Continue() {
    document.querySelectorAll('#step-1-fields .text-danger.small').forEach(el => el.classList.add('d-none'));

    let isValid = true;

    const name = document.getElementById('reg-name').value.trim();
    if (!/^[A-Za-z\s\.]{2,100}$/.test(name)) {
        document.getElementById('err-reg-name').classList.remove('d-none');
        isValid = false;
    }

    const dept = document.getElementById('reg-dept').value;
    if (!dept) {
        document.getElementById('err-reg-dept').classList.remove('d-none');
        isValid = false;
    }

    const contact = document.getElementById('reg-contact').value.trim();
    if (!/^[6-9]\d{9}$/.test(contact)) {
        document.getElementById('err-reg-contact').classList.remove('d-none');
        isValid = false;
    }

    const email = document.getElementById('reg-email').value.trim();
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        document.getElementById('err-reg-email').classList.remove('d-none');
        isValid = false;
    }

    const qual = document.getElementById('reg-qual').value.trim();
    if (!qual) {
        document.getElementById('err-reg-qual').classList.remove('d-none');
        isValid = false;
    }

    const exp = parseInt(document.getElementById('reg-exp').value, 10);
    if (isNaN(exp) || exp < 0 || exp > 70) {
        document.getElementById('err-reg-exp').classList.remove('d-none');
        isValid = false;
    }
...
function handleRegisterSave() {
    ...
    if (!/^[a-zA-Z0-9_]{4,50}$/.test(user)) {
        document.getElementById('err-reg-user').classList.remove('d-none');
        isValid = false;
    }
    ...
    if (!passRegex.test(pass)) {
        document.getElementById('err-reg-pass').classList.remove('d-none');
        isValid = false;
    }

    const passConfirm = document.getElementById('reg-pass-confirm').value;
    if (pass !== passConfirm) {
        document.getElementById('err-reg-pass-confirm').classList.remove('d-none');
        isValid = false;
    }
...
function openSlotsViewer(id) {
    ...
    // Render Shifts
    const shifts = StorageDB.getDoctorShifts(id);
    const shiftsBody = document.getElementById('slots-shifts-tbody');
    shiftsBody.innerHTML = '';

    if (shifts.length === 0) {
        shiftsBody.innerHTML = `<tr><td colspan="4" class="text-center text-muted">No active shifts defined.</td></tr>`;
    } else {
        shifts.forEach(s => {
            shiftsBody.innerHTML += `
                <tr>
                    <td class="px-3 fw-medium">${s.shiftName}</td>
                    <td class="px-3">${s.start}</td>
                    <td class="px-3">${s.end}</td>
                    <td class="px-3">${s.duration}</td>
                </tr>
            `;
        });
    }
