...
function showScreen(screenId) {
  ...
  if (screenId === 'screen-holidays') {
    subtitle.innerText = 'Declare Hospital Holiday';
    updateMicroBreadcrumb('Manage Holidays');
    resetHolidayForm();
    renderHolidaysList();
  }
}
...
function resetHolidayForm() {
  ...
  document.getElementById('holiday-date-error').textContent = 'Enter a valid date in DD-MM-YYYY or YYYY-MM-DD format.';
  ['holiday-date', 'holiday-name', 'holiday-reason'].forEach(id => {
...
function renderHolidaysList() {
  ...
  const holidays = StorageDB.getHolidays().slice().sort((a, b) => a.date.localeCompare(b.date));
  if (!holidays.length) {
    tbody.innerHTML = `<tr><td colspan="4" class="text-center text-muted py-4">No holidays declared yet.</td></tr>`;
    return;
  }
  tbody.innerHTML = holidays.map(h => `
    <tr>
      <td class="px-3">${StorageDB.escapeHtml(h.date)}</td>
      <td class="px-3">${StorageDB.escapeHtml(h.name)}</td>
      <td class="px-3">${StorageDB.escapeHtml(h.reason)}</td>
      <td class="px-3"><button class="btn btn-sm btn-outline-danger" onclick="handleDeleteHoliday('${h.id}')">Delete</button></td>
    </tr>
  `).join('');
}
...
function handleAddHoliday() {
  ...
  const dateErr = document.getElementById('holiday-date-error');
  const nameErr = document.getElementById('holiday-name-error');
  const reasonErr = document.getElementById('holiday-reason-error');

  let valid = true;
  const PV = window.ProfileValidation;
  const setState = PV ? PV.setFieldState : function(el, errEl, ok) {
    if (ok) { el.classList.remove('is-invalid'); el.classList.add('is-valid'); if (errEl) errEl.classList.add('d-none'); }
    else { el.classList.remove('is-valid'); el.classList.add('is-invalid'); if (errEl) errEl.classList.remove('d-none'); }
  };

  const normalizedDate = StorageDB.normalizeHolidayDate(dateInput.value.trim());
  if (!normalizedDate) {
    setState(dateInput, dateErr, false);
    valid = false;
  } else {
    setState(dateInput, dateErr, true);
  }

  const name = nameInput.value.trim();
  if (!name || name.length > 60) {
    setState(nameInput, nameErr, false);
    valid = false;
  } else {
    setState(nameInput, nameErr, true);
  }

  const reason = reasonInput.value.trim();
  if (!reason || reason.length > 200) {
    setState(reasonInput, reasonErr, false);
    valid = false;
  } else {
    setState(reasonInput, reasonErr, true);
  }

  if (!valid) return;

  if (StorageDB.isHoliday(normalizedDate)) {
    dateInput.classList.add('is-invalid');
    dateErr.textContent = 'This date is already declared as a holiday.';
    dateErr.classList.remove('d-none');
    return;
  }

  StorageDB.saveHoliday({ id: StorageDB.generateHolidayId(), date: normalizedDate, name, reason });
  if (StorageDB.addAuditLog) StorageDB.addAuditLog('Admin', 'Create', 'Holiday', `Declared holiday "${name}" on ${normalizedDate}`, 'Info');
  resetHolidayForm();
  renderHolidaysList();
}

function handleDeleteHoliday(id) {
  const holiday = StorageDB.getHolidayById ? StorageDB.getHolidayById(id) : null;
  StorageDB.deleteHoliday(id);
  if (StorageDB.addAuditLog) StorageDB.addAuditLog('Admin', 'Delete', 'Holiday', holiday ? `Removed holiday "${holiday.name}" on ${holiday.date}` : `Removed holiday ${id}`, 'Warning');
  renderHolidaysList();
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── Live validation for holiday form ──
  if (window.ProfileValidation) {
    ProfileValidation.liveValidate([
      { input: 'holiday-date', error: 'holiday-date-error', required: true, label: 'Date', validator: function(v) {
        if (!v || !v.trim()) return 'Date is required.';
        return StorageDB.normalizeHolidayDate(v.trim()) ? '' : 'Enter a valid date in DD-MM-YYYY or YYYY-MM-DD format.';
      }},
      { input: 'holiday-name', error: 'holiday-name-error', required: true, label: 'Holiday Name', validator: function(v) {
        if (!v || !v.trim()) return 'Holiday Name is required.';
        if (v.trim().length > 60) return 'Holiday Name must be 60 characters or less.';
        return '';
      }},
      { input: 'holiday-reason', error: 'holiday-reason-error', required: true, label: 'Reason', validator: function(v) {
        if (!v || !v.trim()) return 'Reason is required.';
        if (v.trim().length > 200) return 'Reason must be 200 characters or less.';
        return '';
      }}
    ]);
  }
