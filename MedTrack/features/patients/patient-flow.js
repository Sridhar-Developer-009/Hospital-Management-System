...
function handlePatientRegisterSave() {
  ...
  const check = function(inputId, errId, validator, label) {
    ...
    if (msg) { if (errEl) errEl.textContent = msg; valid = false; }
  };
  check('reg-patient-name', 'reg-patient-name-error', PV ? PV.validateName : function(v){ return !v ? 'Required.' : ''; }, 'Full Name');
  check('reg-patient-gender', 'reg-patient-gender-error', PV ? PV.validateGender : function(v){ return !v ? 'Required.' : ''; }, 'Gender');
  check('reg-patient-contact', 'reg-patient-contact-error', PV ? PV.validatePhone : function(v){ return !/^[6-9]\d{9}$/.test(v) ? 'Invalid.' : ''; }, 'Contact');
  check('reg-patient-dept', 'reg-patient-dept-error', PV ? PV.validateDepartment : function(v){ return !v ? 'Required.' : ''; }, 'Department');
  if (!valid) return;
...
function resetPatientRegisterForm() {
  ...
  document.querySelectorAll('#form-register-patient .form-control, #form-register-patient .form-select').forEach(el => el.classList.remove('is-valid', 'is-invalid'));
  populatePatientDoctorDropdown();
}
...
function renderViewPatients() {
  ...
  page.forEach((p, idx) => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button class="btn btn-sm btn-outline-primary" onclick="event.stopPropagation();showPatientViewDetail('${p.id}')">View</button>
        <button class="btn btn-sm btn-outline-secondary" onclick="event.stopPropagation();navigateToModifyPatient('${p.id}')">Edit</button>
      </td>
    `;
...
function navigateToModifyPatient(id) {
  showScreen('screen-modify');
  openPatientModify(id);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function consumeSearchNav() {
    ...
    try {
      ...
      if (nav.type === 'Patient' && nav.id) {
        showScreen('screen-view');
        showPatientViewDetail(nav.id);
      }
    } catch (e) {}
  }
  consumeSearchNav();
  ...
  // ── Live validation for patient registration ──
  if (window.ProfileValidation) {
    ProfileValidation.liveValidate([
      { input: 'reg-patient-name', error: 'reg-patient-name-error', required: true, label: 'Full Name', validator: ProfileValidation.validateName },
      { input: 'reg-patient-gender', error: 'reg-patient-gender-error', required: true, label: 'Gender', validator: ProfileValidation.validateGender },
      { input: 'reg-patient-contact', error: 'reg-patient-contact-error', required: true, label: 'Contact', validator: ProfileValidation.validatePhone },
      { input: 'reg-patient-dept', error: 'reg-patient-dept-error', required: true, label: 'Department', validator: ProfileValidation.validateDepartment }
    ]);
  }
});
